...
class ControllerExtensionModuleFeatured extends Controller {
    ...
    public function index($setting) {
            ...
            foreach ($products as $product_id) {
                ...
                if ($product_info) {
                    if ($product_info['fimage']) {
                        ...
                        $image = 'image/' . $product_info['fimage'];
        ...
        $get_cats = array("60", "61");
        foreach ($get_cats as $cid) {
            $category_infos = $this->model_catalog_category->getCategory($cid);
            $catid = $category_infos['category_id'];
            $data['categories'][] = array(
                'cat_id' => $catid,
                'thumb'  => "image/" . $category_infos['image'],
                'name'   => $category_infos['name'],
                'action'   => $this->url->link('product/category', 'path='.$catid)
            );
        }
