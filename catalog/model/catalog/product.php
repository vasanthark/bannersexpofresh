...
class ModelCatalogProduct extends Model {
    ...
    public function getProduct($product_id) {
        ...
        if ($query->num_rows) {
            return array(
                ...
                'footer_keywords' => $query->row['footer_keywords'],
                'tag' => $query->row['tag'],
    ...
    public function getAvgrating($pid) {
        $query = $this->db->query("SELECT AVG(rating) AS avgrate FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = '" . $pid . "' AND r1.status = '1'");
        if ($query->num_rows) {
            return $query->row['avgrate'];
        } else {
            return false;
        }
    }
...
//            echo "<br>";
    ...
    public function sendquote($product_id, $data) {
...
//$header.= "X-Priority: 1\r\n"; 
//$mresult = mail($to,$subject,$txt,$header); 
//if(!$mresult) {   
//     echo "Error";   
//} else {
//    echo "SuccessV3";
...
//exit;
        ...
        $selected_options = $this->productoptions($product_id,$data);    

        $subject = $product_info['name'] . " - Get A Quote By " . html_entity_decode($data['username'], ENT_QUOTES, 'UTF-8');

        $message = "Here are the below quote informations asked by " . html_entity_decode($data['username'], ENT_QUOTES, 'UTF-8'). ". \n\n";  
                 
        $message .= "Name  : " . html_entity_decode($data['username'], ENT_QUOTES, 'UTF-8') . "\n";
        $message .= "Email : " . $data['useremail'] . "\n";
        $message .= "Phone : " . $data['userphone'] . "\n\n";
        if(isset($data['usercomment']) && $data['usercomment']!="")
        $message .= "Comments : " . $data['usercomment'] . "\n\n";
        
        $message .= "Here are the below quote informations asked for '" . $product_info['name'] . "' product" . "\n\n";        
       
        $message .= "Quote Informations :" . "\n";
        if (!empty($selected_options)) {
            foreach ($selected_options as $option) {
                if($option['value']!="")
                $message .=   "\n". $option['name'] . " : " . html_entity_decode($option['value'], ENT_QUOTES, 'UTF-8');
            }
        }
        $message .= "\n";
        $message .= "Height Feet :" .$data['quote_hf']. "\n";
        $message .= "Height Inch :" .$data['quote_hi']. "\n";
        $message .= "Width Feet :" .$data['quote_wf']. "\n";
        $message .= "Width Inch :" .$data['quote_wi']. "\n\n";
        $message .= "Quantity :" .$data['quantity']. "\n\n";
        $message .= "Thanks \n";
        $message .=  html_entity_decode($data['username'], ENT_QUOTES, 'UTF-8');
        ...
        $mail->setFrom($data['useremail']);
        $mail->setSender(html_entity_decode($data['username'], ENT_QUOTES, 'UTF-8'));
        $mail->setSubject($subject);
        $mail->setText($message);
        $mail->send();       
    }

    public function productoptions($product_id,$data) {
        $option_data = array();
        if(isset($data['option']))
        {    
            foreach ($data['option'] as $product_option_id => $value) {

                $option_query = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int) $product_option_id . "' AND po.product_id = '" . (int) $data['product_id'] . "' AND od.language_id = '" . (int) $this->config->get('config_language_id') . "'");
                ...
                if ($option_query->num_rows) {
                    ...
                    if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio') {
                        ...
                        if (is_array($value)) {
                            ...
                            // Double side banner
                            if ($option_query->row['option_id'] == 17) {
                                $product_option_value_id = $option_query->row['option_id'];
                    ...
                    } elseif ($option_query->row['type'] == 'text' || $option_query->row['type'] == 'textarea' || $option_query->row['type'] == 'file' || $option_query->row['type'] == 'date' || $option_query->row['type'] == 'datetime' || $option_query->row['type'] == 'time' || $option_query->row['type'] == 'hidden') {

                        $no_display_contents = array("46", "22");
                        if (!in_array($option_query->row['option_id'], $no_display_contents)) {
                            $option_data[] = array(
                                'product_option_id' => $product_option_id,
    ...
    public function getSpecDownload($product_id) {
