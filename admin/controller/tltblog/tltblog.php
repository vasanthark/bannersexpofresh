...
class ControllerTltBlogTltBlog extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			...
			$data['article_image'] = $this->request->post['article_image'];
		} elseif (!empty($tltblog_info)) {
			$data['article_image'] = $tltblog_info['article_image'];
		} else {
			$data['article_image'] = '';
		}
                
                if (isset($this->request->post['article_image']) && is_file(DIR_IMAGE . $this->request->post['article_image'])) {
			$data['article_thumb_image'] = $this->model_tool_image->resize($this->request->post['article_image'], 100, 100);
		} elseif (!empty($tltblog_info) && is_file(DIR_IMAGE . $tltblog_info['article_image'])) {
			$data['article_thumb_image'] = $this->model_tool_image->resize($tltblog_info['article_image'], 100, 100);
		} else {
			$data['article_thumb_image'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
