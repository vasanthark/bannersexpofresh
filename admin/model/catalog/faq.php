...
class ModelCatalogFaq extends Model {
	public function addFaq($data) {
		...
		foreach ($data['faq_description'] as $language_id => $value) {
                    if($value['keyword'] == ''){
                        $keyword = strtolower(str_replace("?","",str_replace(" ","-",$value['title'])));  
                    }else{
                        $keyword = $value['keyword'];
                    }
			$this->db->query("INSERT INTO " . DB_PREFIX . "faq_description SET faq_id = '" . (int)$faq_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', keyword = '" . $this->db->escape($keyword) . "'");
                        $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'faq_id=" . (int)$faq_id . "', keyword = '" . $this->db->escape($keyword) . "'");
		}
