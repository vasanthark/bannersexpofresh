...
class ModelTltBlogTltBlog extends Model {
	public function addTltBlog($data) {
		...
		if (isset($data['keyword']) && !empty($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_url_alias SET query = 'tltblog_id=" . (int)$tltblog_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
                        $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'tltblog_id=" . (int)$tltblog_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}else{
                     $keyword = 'blog/'.strtolower(str_replace(" ","-",$title));                    
                        $this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_url_alias SET query = 'tltblog_id=" . (int)$tltblog_id . "', keyword = '" . $this->db->escape($keyword) . "'");
                        $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'tltblog_id=" . (int)$tltblog_id . "', keyword = '" . $this->db->escape($keyword) . "'");
                }
	...
	public function editTltBlog($tltblog_id, $data) {
                $title = '';
		$this->db->query("UPDATE " . DB_PREFIX . "tltblog SET sort_order = '" . (int)$data['sort_order'] . "', image = '" . (isset($data['image']) ? $this->db->escape($data['image']) : '') . "', article_image = '" . (isset($data['article_image']) ? $this->db->escape($data['article_image']) : '') . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', show_title = '" . (isset($data['show_title']) ? (int)$data['show_title'] : 0) . "', status = '" . (int)$data['status'] . "', show_description = '" . (isset($data['show_description']) ? (int)$data['show_description'] : 0) . "' WHERE tltblog_id = '" . (int)$tltblog_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "tltblog_url_alias WHERE query = 'tltblog_id=" . (int)$tltblog_id . "'");
                $this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'tltblog_id=" . (int)$tltblog_id . "'");
	...
	public function getTotalTltBlogsByLayoutId($layout_id) {
        ...
        public function editTltBlogReview($review_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "tltblog_review SET  status = '" . (int)$data['status'] . "' WHERE review_id = '" . (int)$review_id  . "'");
	}

	public function deleteReview($review_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "tltblog_review WHERE review_id = '" . (int)$review_id . "'");
	}

	public function getReview($review_id) {
		$query = $this->db->query("SELECT r.*,bd.title FROM " . DB_PREFIX . "tltblog_review as r  LEFT JOIN " . DB_PREFIX . "tltblog_description bd ON (r.tltblog_id = bd.tltblog_id) WHERE r.review_id = '" . (int)$review_id . "'");

		return $query->row;
	}

	public function getReviews($data = array()) {
		$sql = "SELECT r.*,bd.title FROM " . DB_PREFIX . "tltblog_review as r  LEFT JOIN " . DB_PREFIX . "tltblog_description bd ON (r.tltblog_id = bd.tltblog_id) ORDER BY review_id DESC";		
	...
	public function getTotalReviews($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "tltblog_review";
